...
Sanzhi has spatial and non-\isi{spatial postpositions}. Some of the \isi{spatial postpositions} also have temporal readings. The majority of the \isi{spatial postpositions} are widely used as adverbs and then occur without a dependent \isi{noun phrase} (\refsec{ssec:SpatialAdverbsDerivedFromPostpositions}). Thus, the distinction between postpositions and adverbs is rather blurred. The distinction between postpositions and \isi{spatial cases} is, by contrast, relatively clear-cut with respect to the morphosyntax, although there are no clear intonational and often also no clear semantic differences. Most postpositions govern the \isi{genitive} case; otherwise two \isi{spatial cases} or the \isi{absolutive} case are used (\reftab{tab:Spatial postpositions}). This is in contrast to \isi{spatial cases}, which are suffixed directly to the nominal stem or to the oblique/\isi{ergative} suffix. Furthermore, only the postposition \textit{sa} has a clear cognate form used as \isi{spatial case} (\refsec{sssec:ante-lative -sa, ante-essive -sa-b, and ante-\isi{ablative} -sa-r}). The postposition \textit{sa} is shown in \refex{in front of the house}; examples with the cognate \isi{spatial case} are given in \refex{In front of his house there is also a good area, he says} and \refex{he himself also slept in front of it}. 
%
\begin{exe}
	\ex	
	\begin{xlist}
...
There is another class of morphemes with which postpositions formally and semantically overlap, namely spatial \isi{preverbs}. The postpositions \textit{sa}, \textit{hitːi}, \textit{či}, \textit{b}-\textit{i} and \textit{tːura} also occur as location \isi{preverbs} (\refsec{ssec:Location \isi{preverbs} and \isi{spatial cases} expressing direction}) that can be combined with the postposition or case marker or occur on their own. In example \refex{he himself also slept in front of it} both the \isi{spatial case} \textit{-sa} and the \isi{preverb} \textit{sa-} are used.

This chapter explores \isi{spatial postpositions} (including those with temporal meanings) (\refsec{sec:Spatialpostpositions}) and non-\isi{spatial postpositions} (\refsec{sec:Non-spatial postpositions}). 
...
In addition, \textit{hitːi} is widely used as a \isi{temporal adverb} (\refsec{sec:TemporalAdverbs}), including temporal \isi{adverbial clauses} (\refsec{sec:temporalcausal postposition hiti}), and the short encliticized version \textit{=itːi} occurs within \isi{compound verbs} (\refsec{ssec:compoundswithnouns}, example \refex{ex:markednouncpafter}).
...
This postposition governs the \isi{genitive} \xxref{Hungry and wild, we went out of the village}{also all people outside of the villag}. However, it more frequently occurs as an adverb and as a spatial \isi{preverb} with preceding \isi{nouns} in the \textsc{in}-\isi{ablative} or \textsc{loc}-\isi{ablative} (\refsec{ssec:SpatialAdverbsDerivedFromPostpositions}, \refsec{ssec:Location \isi{preverbs} and \isi{spatial cases} expressing direction}).
%
\begin{exe}
	\ex
	\begin{xlist}
