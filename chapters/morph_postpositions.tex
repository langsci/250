...
There is another class of morphemes with which postpositions formally and semantically overlap, namely spatial \isi{preverbs}. The postpositions \textit{sa}, \textit{hitːi}, \textit{či}, \textit{b}-\textit{i} and \textit{tːura} also occur as location \isi{preverbs} (\refsec{ssec:Location preverbs and spatial cases expressing direction}) that can be combined with the postposition or case marker or occur on their own. In example \refex{he himself also slept in front of it} both the \isi{spatial case} \textit{-sa} and the \isi{preverb} \textit{sa-} are used.
