...
Other negative \is{indefinite pronoun}indefinite pronouns are \tit{caʔarra} \sqt{no one} and \tit{cajnara} \sqt{never, not once} (\tit{ca-jna=ra} one-\tsc{time}\tsc{=add}). The first pronoun consists of (\tit{ca-ʔar=ra} one-?\tsc{=add}) and seems to be related the \isi{focus-sensitive particle} \tit{arrah} \sqt{at least} (\refsec{ssec:Further enclitics that manipulate the information structure})
