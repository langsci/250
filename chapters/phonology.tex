...
The phonemic glottal stop is found in the noun \tit{beʔe} \sqt{blood} and at the end of some words, for instance in the root-final position of two verbs \tit{ha-ʔ-} (\tsc{pfv}) / \tit{h-erʔ-} (\tsc{ipfv}) \sqt{say}, and \tit{b-erʔ-} (\tsc{pfv}) / \tit{b-uʔ-} (\tsc{ipfv}) \sqt{rot} and the numeral \tit{kːaʔ-al} \sqt{eight}. Except for \tit{beʔe} \sqt{blood}, only loan words and names contain the glottal stop in root-medial position, e.g. \tit{žaˁbraˁʔil} (male name), \tit{daʔim} \sqt{continuation}.
...
\begin{exe}
	...
	\begin{xlist}
		\ex	\tit{b-uˁc-ce} / \tit{b-uˁc-te} \sqt{thick} (\tsc{n-}thick\tsc{-dd.sg} / \tsc{n-}thick\tsc{-dd.pl})	\label{ex:bucce phon}
		\ex	\tit{tunt-ce} / \tit{tunt-te} \sqt{daring}	\label{ex:tuntce phon}
		\ex	\tit{ik'-ka} / \tit{hek'-ka} \sqt{\tsc{dem.up}\tsc{-abl}} (alternatively \tit{ik-ka} / \tit{hek-ka})	\label{ex:ikka phon}
	\end{xlist}
\end{exe}
...
\begin{exe}
	...
	\TabPositions{11em}
		...
		\tit{-lla / -la} (genitive suffix)	\tab \tit{-lle / -le} (adverbialzing suffix)	\\
		\tit{urra} \sqt{foreign}	
	%\end{xlist}
...
The minimal word (i.e. free root) has the shape V (see the example in \refex{ex:syllable V phon} above). Minimal bound roots seem to consist of a single consonant and are only found among verbs. Examples are \tit{ha-ʔ-} (\tsc{pfv}) \sqt{say} and \tit{ka-xʷ-} / \tit{ha-xʷ-} (\tsc{pfv}) \sqt{pour, add}. These verbs obligatorily contain preverbs and the vowel can be analyzed as either belonging to the preverb (which results then in the monosegmental verb stems) or to the verbs, or two both (\tit{ha-} + \tit{aʔ-} > \tit{haːʔ-} > \tit{haʔ-}).
...
\begin{exe}
	...
	\begin{xlist}
	\TabPositions{15em}
		\ex	\tit{ruˁqː-uˁl} / \tit{ruˁqː-ul} \sqt{educate\tsc{-icvb}} \tab \tit{r-uqː-ul} \sqt{\tsc{f-}bring\tsc{-icvb}}
...
\begin{exe}
	...
	\begin{xlist}
		\ex	\tit{puˁšːuˁk'} / \tit{puˁšːuk'} \sqt{blister}
		\ex	\tit{maˁlʡuˁn} / \tit{maˁlʡun} \sqt{snake}
		\ex	\tit{ʡuˁruˁs} / \tit{ʡuˁrus} \sqt{Russian}
	\end{xlist}

	\ex	{pharyngealization of second vowel or both vowels}\label{ex:pharyngealization of second vowel or both vowels}
	\begin{xlist}
		\ex	\tit{daˁʡaˁna} / \tit{daʡaˁna} \sqt{secret, secretly}
		\ex	\tit{durħuˁ} / \tit{duˁrħuˁ} \sqt{boy, son}
	\end{xlist}
\end{exe}
...
\begin{exe}
	...
	\begin{xlist}
		\ex	\tit{duħi} / \tit{duˁħi} \sqt{snow}
		\ex	\tit{zaˁnʁ} / \tit{zanʁ} \sqt{ring} (ideophone)
		\ex	\tit{čaˁʁir} / \tit{čaʁir} \sqt{wine}
	\end{xlist}
\end{exe}
...
\subsection{Alternations in the form of enclitics / suffixes}
\label{ssec:Alternations in the form of enclitics / suffixes}

There are suffixes and enclitics that have allomorphs whose use depends on the syllable structure of the item to which the suffixes or enclitics are added. The general rule is that suffixes / enclitics consisting of a single consonant are attached to vowel-final words and suffixes / enclitics of the form CV to consonant-final words. Relevant suffixes and enclitics are:
%
\begin{itemize}
	\item	the ergative suffix \tit{-l} / \tit{-li} (\refsec{sssec:Ergative}), e.g. \textit{du-l} (\tsc{1sg-erg}) vs. \textit{kulpat-li} (family-\tsc{erg})
	\item	the adverbializer \tit{-l} / \tit{-le} (\refsec{ssec:The adverbializer -le}), e.g. \textit{razi-l} \sqt{happily} vs. \textit{c'aq'-le} \sqt{strongly}
	\item	the emphatic enclitic \tit{=n} / \tit{=nu} (\refsec{ssec:The enclitic =n(u)}), e.g. \textit{akːu=n} (be\tsc{.neg.prs=prt}) vs. \textit{le-d=nu} (exist-\tsc{npl=prt})
	\item	the enclitic for polar questions \tit{=w} / \tit{=uw} (\refsec{sec:Simple polar questions and disjunctive polar questions}) \textit{arg-ul=de=w?} (go.\tsc{ipfv-icvb=2sg=q}) \sqt{Are you going?} vs. \textit{le-b=uw?} (exist-\tsc{n=q}) \sqt{Does it exist?}
\end{itemize}
...
%
\begin{itemize}
	\item	the derivational suffix used to form the numerals 2\tnd10, 20, as well as 100: \tit{-al} / \tit{-jal}, e.g. \tit{aʁʷ-al} \sqt{four} vs. \tit{xu-jal} \sqt{five} 
	\item	the derivational suffix \sqt{X-times} for the formation of multiplicative numerals \tit{-na} / \tit{-jna} 
	\item	one of the allomorphs of the spatial case suffix of the \tsc{loc}-series: \tit{-a} / \tit{-ja} 
	\item	optional marker for non-indicative verb forms that  serves as address particle for plural addressees \tit{-a} / \tit{-ja}, e.g. in the imperative plural suffix \tit{-ene-ja} (alternative variant of \tit{-ene}); the particle \tit{ma} vs. \tit{ma-ja} `Here, take!'
...
\begin{exe}
	...
	\begin{xlist}
		...
		\ex	\tit{a-erč-ur} > \tit{eːrčur} / \tit{aʔerčur} \sqt{did not saw} (\tsc{neg-}saw\tsc{.pfv-pret})
...
\begin{exe}
	...
	\begin{xlist}
		\ex	\tit{han a-w-irk-u} > \tit{han awirku} / \tit{han.erku} / \tit{han.aʔirku} \sqt{does not remember\newline\hspace*{1em}him} (remember \tsc{neg-m-}occur\tsc{.ipfv-prs.3})  
		\ex	\tit{a-irʁ-ib=da} > \tit{erʁibda} / \tit{aʔirʁibda} \sqt{I did not understand}\newline\hspace*{1em}(\tsc{neg-}understand\tsc{.pfv-pret=1})
	\end{xlist}
\end{exe}
%
...
\begin{exe}
	...
	\begin{xlist}
		\ex	\label{ex:sawPHON} \tit{a-erč-ur} > \tit{aʔerčur} / \tit{eːrčur} (\tsc{neg-}saw\tsc{.pfv-pret})
...
%
\begin{itemize}
	\ex	genitive case: \tit{-la} > \tit{-na} / \tit{-ra}
	\ex	\tsc{loc}-series (spatial case): \tit{-le} > \tit{-ne} / \tit{-re}
	\ex	ergative case / oblique stem marker: \tit{-li} > \tit{-ni} / \tit{-ri}
	\ex	perfective converb / adverbializer: \tit{-le} > \tit{-ne} / \tit{-re}
	\ex	anteriority / causality converb \tit{-la} > \tit{-na} / \tit{-ra}
\end{itemize}
%
\begin{exe}
	\ex	{\tit{n} + \tit{l} > \tit{nn}; \tit{r} + \tit{l} > \tit{rr}}\label{ex:n l nn r l rr phon}
	\begin{xlist}
		\ex	\tit{cin-la} > \tit{cinna} \sqt{his / her} (\tsc{refl.sg.obl-gen})
...
\begin{exe}
	...
	\begin{xlist}
		\ex	\tit{ubč'-ni} / \tit{ubk'-ni} (die\tsc{.m.ipfv-msd}) < \tit{b-ubk'-} (\tsc{n-}die\tsc{.ipfv-}) 
		\ex	\tit{b-arč-ni} / \tit{b-ark-ni} (\tsc{n-}find\tsc{.pfv-msd}) < \tit{b-arkː-} (\tsc{n-}find\tsc{.pfv-})
	\end{xlist}
\end{exe}
...
\begin{exe}
	...
	\begin{xlist}
		...
		\ex	\tit{akːʷ-} (be\tsc{.neg}) > \tit{akʷ-ni} / \tit{akʷ-ri} (masdar)
