...
As mentioned in \refsec{ssec:Scope properties} above, interrogative \is{particle}particles (which also belong to the focus-sensitive \is{particle}particles) cannot be used in \is{adverbial clause}adverbial clauses. However, \is{adverbial clause}adverbial clauses with various converbs can contain interrogative pronouns as the following examples with the \isi{perfective converb} \refex{ex:When Hazhimurad was given what we were happy} and the converb \tit{=qːel} \refex{ex:‎When who is guiding the car the girls get afraid} show. 
%
\begin{exe}
