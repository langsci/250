...
Adverbial clauses do not contain markers for illocutionary force, such as the imperative, optative suffixes, or the interrogative particles (\sqt{banned}). Those markers can only occur in the main clause. Their scope can be restricted to the main clause (\sqt{local}), but, in the appropriate context, it can also extent across the adverbial clause (\sqt{extensible}). However, the latter possibility is noticeably less common in texts. Examples \refexrange{ex:When you put the roof beam at this (at one) side}{ex:‎‎The legs are trembling, jump} illustrate local scope restricted to the main clause.
%
\begin{exe}
	...
	\gll	[hej	šːal-li-cːe	cːiχːin	ka-b-alt-an=qːel]	het	šːal-la	ʡaˁnčːi	a-ka-d-ax-u=w?\\
		this	side\tsc{-obl-in}	roof.beam	\tsc{down-n-}put\tsc{.ipfv-ptcp=}when	that	side\tsc{-gen}	earth(\tsc{npl})	\tsc{neg-down}\tsc{-npl}-go-\tsc{prs.3=q}\\
...
\begin{exe}
	...
	\gll	[it-i-sa-r	s-asː-ib-le	wec'-nu	urek-ra	azir]	it-i-l=ra d-ataʁ-ib-le=de\\
		that\tsc{-obl-ante-abl}	\tsc{hither}-take\tsc{.pfv-pret-cvb}	ten-\tsc{tens}	six\tsc{-num}	thousand that\tsc{-obl-erg=add}	\tsc{npl-}send\tsc{.pfv-pret-cvb=pst}\\
	\glt	\sqt{From her (he) took 16\thinspace 000, and he sent (that money to us).}
\end{exe}

In short, fewer categories are expressed in adverbial clauses than in main clauses, because person agreement, tense, evidentiality, and illocutionary force are absent. This means that Sanzhi adverbial clauses are, in Bickel's terms, \sqt{asymmetrical} and non-finite \citep{Bickel2010}. 
...
This criterion concerns the linear order of adverbial clause and main clause (\sqt{position} and \sqt{layer} in the terminology of \citealp{Bickel2010}). Although the adverbial clauses most frequently precede the main clause, they may also follow it \refex{ex:‎‎‎I remained there for two years, unable to eat food made of corn}-\refex{ex:‎‎‎I came to know the truth from you, I said, because he (the other doctor) did not tell me (the truth)}, \refex{ex:‎When they did not calm down, (he) put them into the box, frightening them}, and they may be separated by other subordinate clauses from the main clause, e.g. by other adverbial clauses.
...
\begin{exe}
	...
	\gll	c'il	roddom-le	heštːu-d	lukː-unne=kːu=w	ce=ja	arc	[tːura	h-asː-ij	r-ax-an=qːel]?\\
		then	maternity.hospital\tsc{-loc}	here\tsc{-npl}	give\tsc{.ipfv-icvb=}be\tsc{.neg=q}	what\tsc{=q}	money		outside	\tsc{up}-take\tsc{.pfv-inf}	\tsc{f-}go\tsc{-ptcp=}when\\
...
\begin{exe}
	...
	\glt	\sqt{‎When / Because Murad got ill he did not build the fence.} (E)
...
\begin{exe}
	...
	\glt	\sqt{‎When / Because Murad got ill he (= Murad or some other person) did not build the fence.} (E)
\end{exe}
...
\begin{table}
	...
	\begin{tabularx}{0.88\textwidth}[]{%
		...
		\lsptoprule
			Variable			&	\tsc{ipfv} / \tsc{pfv} converb	&	\tit{=qːel}\\
		\midrule
