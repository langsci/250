...
\label{sec:Gender/\isi{number} agreement}
\subsection{General remarks on gender/number agreement}
\label{General remarks on \isi{gender}/\isi{number} agreement} 
...
\begin{itemize}[leftmargin=*]
	...
	\item a few \isi{compound verbs} with bound lexical stems (e.g. \textit{b-al} ‘together', \textit{b-at} ‘set free, let'), the spatial \isi{preverbs} \textit{b-i-} `in, inside' and \textit{b-it}- ‘thither' (\refsec{sec:Preverbs})
	...
	\item	all items that can be inflected for the essive case, e.g. \isi{nouns}, pronouns, \isi{spatial adverbs}, postpositions, and all items that inflect for the directional case, i.e. mostly \isi{spatial adverbs} (\refsec{sec:nouncase})
\end{itemize}
\end{description}
...
\begin{itemize}[leftmargin=*]
	...
	\item	the \isi{quantifier} \textit{li<b>il} ‘all' and \isi{group numerals} (\refsec{sec:groupnumerals})
...
\begin{exe}
	...
	\begin{xlist}
		\TabPositions{13em}
		\ex	\tit{Ø-uqna admi} \sqt{old person}	\tab	\tit{b-uqna adimte} \sqt{old people}	\label{ex:agreement \isi{number}@A}
		\ex	\tit{r-uqna xːunul} \sqt{old woman}	\tab	\tit{b-uqna xːunre} \sqt{old women}	\label{ex:agreement \isi{number}@B}
		\ex	\tit{b-uqna χːʷe} \sqt{old dog}		\tab	\tit{d-uqna χːude} \sqt{old dogs}		\label{ex:agreement \isi{number}@C}
	\end{xlist}
...
\begin{table}
	...
	\label{tab:The \isi{optative}, \isi{imperative}, and prohibitive}
	\small
...
%
\begin{exe}
	\ex	\label{ex:\isi{habitual present} stem augmentation@20}
	\begin{xlist}
...
There are other monovalent predicates that are \isi{compound verbs}, and that from a morphological point of view contain petrified nominal arguments which in some cases control \isi{gender} agreement and in others do not. These behave just like any other monovalent predicate, i.e. the single argument controls the \isi{person agreement} \refex{ex:‎‎Come on, we will eat future}; see also \refex{ex:I am angry@b} above.
%
\begin{exe}
...
In clauses with \isi{bivalent verbs} that are either genuine \isi{transitive verbs} or \isi{affective verbs} both arguments (i.e. agents\slash experiencers, and patients\slash stimuli) can control \isi{person agreement}, but only one argument at a time. 
...
The four TAM forms are analytic and make use of either the \isi{cross-categorical suffixes} \tit{-ce} or \tit{-il}, which, among other things, are used for the formation of referential attributes that have the morphosyntactic properties of nominals (e.g. headless \isi{relative clauses}). These constructions therefore resemble biclausal constructions, but a detailed investigation is needed before any conclusions can be made.
