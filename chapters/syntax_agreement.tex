...
The person enclitics belong to the predicative \is{particle}particles (\refsec{sec:Predicative particles}). They are normally added to the predicate, but, just as other predicative \is{particle}particles, can also be used to express term focus (also called ``\isi{constituent focus}''). In this case, they are encliticized to the item in focus, which can be an argument or adjunct, such that agreement targets are not only verbs but can be also nominals, adverbs, or other items (\citealp{Kalinina.Sumbatova2007}, \citealp{Sumbatova2013}, \citealp{Forker2016a}).
