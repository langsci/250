...
The chapter closes with a short discussion of constructions that syntactically do not represent complementation, but semantically resemble complement constructions. Parentheticals (\refsec{ssec:Parentheticals}), nominalized relative clauses (\refsec{ssec:Nominalized relative clauses resembling complement constructions}), and adverbial clauses (\refsec{ssec:Adverbial clauses used with emotion and cognition predicates}) belong to these constructions.
...
Not all utterance verbs take complement clauses that represent reported speech. Some rather denote actions that involve speech (e.g. \sqt{teach}, \sqt{command, order}) or they denote specific sounds that imitate speech sounds (e.g. \tit{t'irt'ir b-ik'ʷ-} \sqt{chat}). In \refsec{sec:Reported speech constructions}, I will only discuss constructions containing quotes.
...
Manipulative verbs typically have subjects that differ from the subjects in the complement clause and the semantics of the complement clause is irrealis. Utterance verbs belong to this group, such as \sqt{command, order} and \sqt{teach} and the basic verb of speech \tit{b-ik'ʷ-} (\tsc{ipfv}) \sqt{say}, which is frequently used with a manipulative meaning. Other manipulative verbs are:
%
\begin{exe}
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item	cross-categorical suffix \tit{-ce} (\refsec{ssec:The attributive marker -ce (-te)COMPL})
...
A number of the grammatical markers listen above also occur in other types of subordinate clauses: the perfective and the imperfective converb head adverbial clauses (\refsec{sec:The syntax of adverbial clauses}). The cross-categorical suffix \tit{-ce} occurs in relative clauses (\refcpt{cpt:Relative clauses}). Therefore, it is not always easy to tell apart complement constructions from adverbial or relative clauses.
...
Note that the suffix -\textit{ce} also occurs in nominalized relative clauses that semantically resemble complement clauses of the activity type. These constructions are discussed in \refsec{ssec:Nominalized relative clauses resembling complement constructions}.
...
The verb \tit{b-ik'ʷ-} has grammaticalized into a quotative particle (see below). Moreover, it can express hearsay evidentiality.
...
Other common simple utterance verbs are the transitive verbs \tit{b-urs-} and \tit{b-ux-}, which both can be translated with \sqt{tell}, and the transitive verb \tit{xar b-eʁ-} (\tsc{pfv})\slash\tit{xar b-irʁ-} (\tsc{ipfv}) \sqt{ask}, which occur, like all verbs of speech, with or without a quotative particle.
%
\begin{exe}
...
Finally, the topic of a conversation or a thought can be expressed by using the postposition \tit{qari=či-b} \sqt{on.top=on\tsc{-n}} together with a complement clause bearing the genitive case suffix (see \refex{ex:He thinks about what he had done} and \refsec{ssec:postposition qari} for another example).
...
For the verb  \tit{b-iχʷ-} (\tsc{pfv})\slash\tit{b-irχʷ-} (\tsc{ipfv}) \sqt{be, become, happen, can, be able} the data are not so clear. Some examples show a very flexible word order, which points towards a monoclausal analysis with a periphrastic predicate:
%
\begin{exe}
	\ex
