...
\begin{exe}
	...
	\begin{xlist}
		...
		\ex	\tit{haʔ-} (\tsc{pfv}) / \tit{herʔ-} (\tsc{ipfv}) \sqt{say}
...
\begin{exe}
	...
	\begin{xlist}
		\ex	\tit{xar b-eʁ-} (\tsc{pfv}) / \tit{xar b-irʁ-} (\tsc{ipfv}) \sqt{ask}			
		\ex	\tit{ʁaj (ka-)b-ik'ʷ-} (\tsc{ipfv}) \sqt{quarrel, scold, argue, discuss, talk}
		\ex	\tit{ʁaj b-arq'-} (\tsc{pfv}) / \tit{ʁaj b-irq'-} (\tsc{ipfv}) \sqt{say, tell}
		\ex	\tit{ʁaj b-ikː-} (\tsc{pfv}) / \tit{ʁaj lukː-} (\tsc{ipfv}) \sqt{promise}
		\ex	\tit{čːal b-uq-} (\tsc{pfv}) / \tit{čːal b-ulq-} (\tsc{ipfv}) \sqt{argue, quarrel}
		\ex	\tit{anru b-ikː-} (\tsc{pfv}) / \tit{anru lukː-} (\tsc{ipfv}) \sqt{command, order}
		\ex	\tit{bursːi b-arq'-} (\tsc{pfv}) / \tit{bursːi b-irq'-} (\tsc{ipfv}) \sqt{teach}
		\ex	\tit{waˁw b-ik'ʷ- (\tsc{ipfv}); waˁw haʔ-} (\tsc{pfv}) / \tit{waˁw herʔ-} (\tsc{ipfv}) \sqt{call, cry}
...
\begin{exe}
	...
	\begin{xlist}
		...
		\ex	\tit{razi b-iχʷ-} (\tsc{pfv}) / \tit{razi b-irχʷ-} (\tsc{ipfv}) \sqt{be happy, agree}
		\ex	\tit{xul b-ik'ʷ-} (\tsc{ipfv}) \sqt{dream, wish, hope}
		\ex	\tit{urk' b-uq-} (\tsc{pfv}) / \tit{urk' b-ulq-} \sqt{be frightened, astonished, wonder}
		\ex	\tit{tamaša b-arq'-} (\tsc{pfv}) / \tit{tamaša b-irq'-} (\tsc{ipfv}) \sqt{wonder}
		\ex	\tit{uruχ b-ik'ʷ-} (\tsc{ipfv}) \sqt{get afraid}, \tit{uruχle ca-b} \sqt{be afraid}
		\ex	\tit{uruc b-iχʷ-} (\tsc{pfv}) / \tit{uruc b-irχʷ-} (\tsc{ipfv}) \sqt{get embarrassed, ashamed}, \tit{uruc ca-b} \sqt{be embarrassed, ashamed}
		\ex	\tit{c'aχ ka-b-icː-} (\tsc{pfv}) / \tit{c'aχ ka-b-ircː-} (\tsc{ipfv}) \sqt{be embarrassed, ashamed}
		\ex	\tit{pašman b-iχʷ-} (\tsc{pfv}) /  \tit{pašman b-irχʷ-} (\tsc{ipfv}) \sqt{be sad about, regret}
		\ex	\tit{q'as b-arq'-} (\tsc{pfv}) / \tit{q'as b-irq'-} (\tsc{ipfv}) \sqt{decide}
		\ex	\tit{sa-b-ag-} (\tsc{pfv}) / \tit{sa-b-arg-} (\tsc{ipfv}) \sqt{imagine, envisage, see}
	\end{xlist}
\end{exe}
...
\begin{exe}
	...
	\begin{xlist}
		\ex	\tit{b-aχ-} (\tsc{pfv}) / \tit{b-alχ-} (\tsc{ipfv}) \sqt{get to know, know}
		\ex	\tit{arʁ-} (\tsc{pfv}) / \tit{irʁ-} (\tsc{ipfv}) \sqt{understand} (can be used together with \tit{urk'i} \sqt{heart})
		\ex	\tit{b-elč'-} (\tsc{pfv}) / \tit{b-uč'-} (\tsc{ipfv}) \sqt{read} 
	\end{xlist}
\end{exe}
...
\begin{exe}
	...
	\begin{xlist}
		...
		\ex	\tit{b-iχ-(b)-it-ag-} (\tsc{pfv}) / \tit{b-iχ-(b)-it-arg-} (\tsc{ipfv}) \sqt{believe} (compound verb, containing \textit{b-iχː-} \sqt{believe})
		\ex	\tit{b-iχ-či ag-} (\tsc{pfv}) / \tit{b-iχ-či arg-} (\tsc{ipfv}) \sqt{believe} (compound verb, containing \textit{b-iχː-} \sqt{believe})
		\ex	\tit{šak b-ik-} (\tsc{pfv}) / \tit{šak b-irk-} (\tsc{ipfv}) \sqt{guess, suspect, feel}
	\end{xlist}
\end{exe}
...
\begin{exe}
	...
	\begin{xlist}
		\ex	\tit{qum-ert-} (\tsc{pfv}) / \tit{qum-urt-} (\tsc{ipfv}) \sqt{forget}
		\ex	\tit{han b-ik-}, \tit{b-ičaq-}  (\tsc{pfv}) / \tit{han b-irk-}, \tit{b-irčaq-} (\tsc{ipfv}) \sqt{remember, seem to, imagine, think} 
		\ex	\tit{han b-el, han ca-b} \sqt{remember} 
		\ex	\tit{urk'i-le sa-b-eʁ-} (\tsc{pfv}) / \tit{urk'i-le sa-b-irʁ-} (\tsc{ipfv}) \sqt{descend on the heart, remember} 
		\ex	\tit{urk'i-le-b le-b} \sqt{think, have thoughts} (lit. `be on the heart') 
		\ex	\tit{ħaˁsib b-arq'-} (\tsc{pfv}) / \tit{ħaˁsib b-irq'-} \sqt{test, check}
	\end{xlist}
\end{exe}
...
\begin{exe}
	...
	\begin{xlist}
		...
		\ex	\tit{b-at-} (\tsc{pfv}) / \tit{b-alt-} (\tsc{ipfv}) \sqt{let, leave}
		\ex	\tit{iχtijar b-ikː-} (\tsc{pfv}) / \tit{iχtijar lukː-} (\tsc{ipfv}) \sqt{give permission, right}
		\ex	\tit{χajri a-b-iχʷ-} (\tsc{pfv}) / \tit{χajri a-b-irχʷ-} (\tsc{ipfv}) \sqt{forbid}
	\end{xlist}
\end{exe}
...
\begin{exe}
	...
	\begin{xlist}
		\ex	\tit{b-aʔ.ak'-} (\tsc{pfv}) / \tit{b-aʔ.ik'-} (\tsc{ipfv}) \sqt{begin, start}
		\ex \tit{b-aʔ b-axː-} (\tsc{pfv}) / \tit{b-aʔ b-irxː-} (\tsc{ipfv}) \sqt{begin, start}
		\ex	\tit{či-ka-b-iħ-} (\tsc{pfv}) \tit{či-ka-b-irħ-} (\tsc{ipfv}) \sqt{begin, start}
		\ex	\tit{taman b-iχʷ-} (\tsc{pfv}) / \tit{taman b-irχʷ-} (\tsc{ipfv}), \tit{taman b-arq'-} (\tsc{pfv}) / \tit{taman b-irq'-} (\tsc{ipfv}) \sqt{stop, finish}
	\end{xlist}
\end{exe}
...
\begin{exe}
	...
	\begin{xlist}
		\ex	\tit{b-iχʷ-} (\tsc{pfv}) / \tit{b-irχʷ-} (\tsc{ipfv}) \sqt{can, be able}
...
\begin{exe}
	...
	\begin{xlist}
		\ex	\tit{ʡaˁħle ca-b}, \textit{ʡaˁħle ag-} (\tsc{pfv}) / \textit{ʡaˁħle arg-} (\tsc{ipfv}) \sqt{be good}
		\ex	\tit{wahil ca-b}, \textit{wahil ag-} (\tsc{pfv}) / \textit{wahil arg-} (\tsc{ipfv}) \sqt{be bad}
	\end{xlist}
\end{exe}
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item	infinitive \tit{-ij} / subjunctive (\refsec{ssec:Infinitive and subjunctive})
	...
	%
	\begin{itemize}
		\item	imperfective converb \tit{-ul / -unne} (\refsec{ssec:The imperfective converb})
...
\begin{exe}
	...
	\gll	wahi-l=de [Rašid-li ust'ul ʡaˁħ a-b-arq'-ib-le / a-b-arq'-ib-ce / a-b-arq'-ni]\\
		bad\tsc{-adv=pst} Rashid\tsc{-erg} chair good \tsc{neg-n-}do\tsc{.pfv-pret-cvb} / \tsc{neg-n-}do\tsc{.pfv-pret-dd.sg} / \tsc{neg-n-}do\tsc{.pfv-msd}\\
...
\begin{exe}
	...
	\glt	\sqt{Aminat is finishing to wash / washing the clothes.} (E)
...
\begin{exe}
	...
	\glt	\sqt{‎‎‎The fox began betraying / to betray the devil sister.}
\end{exe}
...
\begin{exe}
	...
	\gll	Ruslan-ni	[kiniga	b-elk'-un-il-la]	či-b-arq'-ib	/	či-d-arq'-ib\\
		Ruslan\tsc{-erg}	book	\tsc{n-}write\tsc{.pfv-pret-ptcp-gen}	\tsc{spr-n-}do\tsc{.pfv-pret} / \tsc{spr-npl-}do\tsc{.pfv-pret}\\
...
Another very frequent utterance verb is \tit{-ʔ-} (\tsc{pfv}) / \tit{-erʔ-} (\tsc{ipfv}) \sqt{say}, which is almost always used with the spatial preverb \tit{ha-} \sqt{upwards}, that is \tit{haʔ-} / \tit{herʔ-}. This is a transitive verb that marks the subject, i.e. the speaker, with the ergative. It is mainly used in reported speech constructions with past time reference. Besides that it functions as a quotative particle (see below).
%
\begin{exe}
...
Other common simple utterance verbs are the transitive verbs \tit{b-urs-} and \tit{b-ux-}, which both can be translated with \sqt{tell}, and the transitive verb \tit{xar b-eʁ-} (\tsc{pfv}) / \tit{xar b-irʁ-} (\tsc{ipfv}) \sqt{ask}, which occur, like all verbs of speech, with and without a quotative particle.
%
\begin{exe}
...
\begin{exe}
	...
	\glt	\sqt{‎I know that he will come / came.} (E)
\end{exe}
...
\begin{exe}
	...
	\begin{xlist}
		...
		\glt	\sqt{Ali feared that Madina would not recognize / know him.}  (controllee~=~P) (E)
	\end{xlist}
\end{exe}
...
\begin{exe}	
	...
	\glt	\sqt{‎Brother finished studying / the studies.} (E)
\end{exe}
