...
The chapter closes with a short discussion of constructions that syntactically do not represent complementation, but semantically resemble complement constructions. To these constructions belong parentheticals (\refsec{ssec:Parentheticals}), nominalized relative clauses (\refsec{ssec:Nominalized relative clauses resembling complement constructions}), and adverbial clauses (\refsec{ssec:Adverbial clauses used with emotion and cognition predicates}).
...
Manipulative verbs typically have subjects that differ from the subjects in the complement clause and the semantics of the complement clause is irrealis. Utterance verbs belong to this group, such as \sqt{command, order} and \sqt{teach} and the basic verb of speech \tit{b-ik'ʷ-} (\tsc{ipfv}) \sqt{say}, that is frequently used with a manipulative meaning. Other manipulative verbs are:
%
\begin{exe}
...
For the verb  \tit{b-irχʷ-} \sqt{be, become, happen, can, be able} the data are not so clear. Some examples show a very flexible word order, which points towards a monoclausal analysis with a periphrastic predicate:
%
\begin{exe}
	\ex
