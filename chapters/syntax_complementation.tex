...
%
\begin{exe}
	\ex	\label{ex:\isi{compound verbs} of speak}
	\begin{xlist}
...
Not all \isi{utterance verbs} take complement clauses that represent \isi{reported speech}. Some rather denote actions that involve speech (e.g. \sqt{teach}, \sqt{command, order}) or they denote specific sounds that imitate speech sounds (e.g. \tit{t'irt'ir b-ik'ʷ-} \sqt{chat}). In \refsec{sec:Reported speech constructions}, I will only discuss constructions containing quotes.
...
There are two verbs that can express the meaning `continue', \tit{kelgʷ-} (\tsc{pfv}) \sqt{remain, stay, be} and the defective verb \tit{b-el} \sqt{remain, stay}. Both are used in \isi{periphrastic verb forms}, which are not complement constructions (see \refsec{sec:Verb forms with kelgw- remain} and \refsec{sec:Verb forms with b-el remain, stay}).
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item	\isi{quotative} \isi{particles} \tit{bik'ul, haʔible} (\refsec{ssec:The \isi{quotative} particles})
...
A \isi{number} of the grammatical markers listen above also occur in other types of subordinate clauses: the perfective and the \isi{imperfective converb} head \isi{adverbial clauses} (\refsec{sec:The syntax of adverbial clauses}). The \isi{cross-categorical suffix} \tit{-ce} occurs in \isi{relative clauses} (\refcpt{cpt:Relative clauses}). Therefore, it is not always easy to tell apart complement constructions from adverbial or \isi{relative clauses}.
...
\begin{table}
	...
	\begin{tabularx}{0.70\textwidth}[]{%
		...
		\midrule
			zero					&	{}		&	{}		&	{}		&	y\\	   
			\isi{quotative} \isi{particle}			&	(y)		&	{}		&	y		&	y\\	   
			\isi{cross-categorical suffix} 			&	{}		&	y 		&	y		&	y\\	   
			\isi{masdar}				&	{}		&	{}		&	y		&	{}\\
			\isi{perfective converb}			&	{}		&	y		&	y		&	{}\\
			\isi{infinitive}\slash \isi{subjunctive}		&	y		&	{}		&	{}		&	{}\\				   
			\isi{embedded question marker}	&	y		&	y		&	{}		&	y\\	   
			\isi{imperfective converb}		&	y		&	y		&	?	&	{}\\		   
		\lspbottomrule
	\end{tabularx}
\end{table}
...
\label{ssec:The \isi{quotative} particles}

Sanzhi has two \isi{quotative} \isi{particles}, \tit{bik'ul} and \tit{haʔible}. They are discussed in more detail in \refsec{sec:Reported speech constructions} in reference to \isi{reported speech}, because they constitute a major encoding strategy for complements of \isi{utterance verbs}. Although \tit{bik'ul} and \tit{haʔible} have preserved their verbal properties (e.g. inflectional morphology, \isi{gender} agreement, position) I will refer to them as ``\isi{particles}'' when they occur in addition to matrix verbs of speech in order to differentiate them from the matrix verbs that take complement clauses. They can therefore be called ``\isi{parentheticals}'' that do not realize syntactic subordination but pragmatically mark a clause as a speech report.
...
Note that the suffix -\textit{ce} also occurs in nominalized \isi{relative clauses} that semantically resemble complement clauses of the activity type. These constructions are discussed in \refsec{ssec:Nominalized \isi{relative clauses} resembling complement constructions}.
...
The following two corpus examples illustrate embedded disjunct polar \isi{questions}. They have the same structure as the embedded polar \isi{questions} with the only difference that there is only one embedded clause and not two. 
...
Other common simple \isi{utterance verbs} are the \isi{transitive verbs} \tit{b-urs-} and \tit{b-ux-}, which both can be translated with \sqt{tell}, and the \isi{transitive verb} \tit{xar b-eʁ-} (\tsc{pfv})\slash\tit{xar b-irʁ-} (\tsc{ipfv}) \sqt{ask}, which occur, like all verbs of speech, with or without a \isi{quotative} \isi{particle}.
%
\begin{exe}
...
The only further peculiarity that \isi{reported speech} construction show, and which they share with other subordinate clauses, most notably other complement clauses, is the use of \isi{reflexive pronouns} as logophors (see \citealp{ForkerSubmittedb} for a detailed account of logophoric reflexives and other properties of non-direct speech constructions in Sanzhi). When the author of the quote, which must be third person, is identical to an argument or adjunct in the quote, the \isi{reflexive pronoun} can be used instead of the first person pronoun \refex{ex:‎‎‎By God, he said that he himself had bought 10 bottles of vodka; and apparently he had brought them with him}. The use of \isi{demonstrative pronouns} is impossible since they would express disjoint reference with the author of the quote.
%
\begin{exe}
...
The use of \isi{reflexive pronouns} in quotes referring to overtly expressed speakers that are first person or second person pronouns is ungrammatical:
%
\begin{exe}
...
Matrix predicates that have \isi{agreement prefixes} and non-\isi{absolutive} arguments usually exhibit local agreement in which the matrix verb agrees with the complement clause as a whole and therefore has the prefix \tit{b-} (neuter singular) \refex{ex:‎I thought that she cannot walk}. This prefix can also be considered to be the default prefix when there is no \isi{agreement controller} (see \refsec{sec:Gender/\isi{number} agreement}).
%
\begin{exe}
...
\label{ssec:Nominalized \isi{relative clauses} resembling complement constructions}
There are a variety of constructions with the predicates listen in \refsec{sec:Complement-taking predicates} above as ``\isi{complement-taking predicates}'', which are syntactically not complement clauses. They function as a core argument of a higher clause and have the internal constituent structure of a clause. But they are usually shorter than real complement clauses and contain only one core argument in addition to the verb. Most importantly, they do not refer to propositions, but to entities such as persons, events, etc. They are, therefore, not complement clauses but headless \isi{relative clauses}, which have been nominalized. Headless \isi{relative clauses} can be formed with (i) the \isi{cross-categorical suffix} -\textit{ce} (plural -\textit{te}), the \isi{cross-categorical suffix} -\textit{il}, and the \isi{modal participle} -\textit{an}. In this section, I will only discuss headless \isi{relative clauses} that occur together with \isi{complement-taking predicates} and show how they differ from true \isi{complementation}. For general information about headless \isi{relative clauses} see \refsec{sec:Headless relative clauses}.
...
The \isi{cross-categorical suffixes} \tit{-il} and -\textit{ce} (as well as the \isi{masdar}) can take case suffixes. Occasionally these nominalized verbs can occur in the argument position of \isi{complement-taking predicates}. For examples, the verb `believe' regularly requires the \isi{dative} case on its \isi{goal} argument. In sentence \refex{ex:‎Nobody believed what had happened to me, there up on the coachwork} the \isi{goal} argument is a nominalized clause with its own arguments and adjuncts.
