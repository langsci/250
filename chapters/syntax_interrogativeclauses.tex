...
\begin{exe}
	...
	\gll	``b-ebč'-ib-le=w?'' Ø-ik'ʷ-ar.	``akːu,''	haʔ-ib=da\\
		\tsc{n-}die\tsc{.pfv-pret-cvb=q}	\tsc{m-}say\tsc{.ipfv-prs} \tsc{be.neg} say\tsc{.pfv-pret=1}\\
...
\begin{exe}
	...
	\glt	\sqt{He is going / not going.} (E)
\end{exe}
...
\begin{exe}
	...
	\gll	``iž	ala	sub-la	bek'	le-b-il=de=w'',	b-ik'-ul	ca<b>i,	``akːʷ-ar-il=de=w?''	\\
		this	\tsc{2sg.gen}	husband\tsc{-gen}	head	exist\tsc{-n-ref=pst=q}	\tsc{hpl-}say\tsc{.ipfv-icvb}	be\tsc{<hpl>}	\tsc{cop.neg-prs-ref=pst=q}\\
...
It is also possible to put the interrogative pronoun in clause-initial position, but this is far less common \refex{ex:‎[He does not leave me alone], what should I do with him}-\refex{ex:Why will he not greet us}. Equally possible and slightly more common is the occurrence of material following the verb.
%
\begin{exe}
...
\begin{exe}
	...
	\gll	hila	qari=či-b	itːi	ʁaj	ka-b-ik'-ul=de?\\
		who.\tsc{gen}	up=on\tsc{-n}	\tsc{3pl}	word	\tsc{down-hpl-}say\tsc{.ipfv-icvb=pst}\\
...
\begin{exe}
	...
	\glt	\sqt{Good morning!} (female / male addressee)
\end{exe}
