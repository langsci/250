...
Valency classes cross-cut the morphological classes of verbs. This means that the morphological classes (underived verb stems with or without \isi{preverbs}, derived verbs, \isi{compound verbs}, see  \refsec{sec:Overview about the general morphological structure of verbs} and \refcpt{cpt:verbformation}) distribute over the \isi{valency classes} with probably a preference for the simple underived verbs to occur in the intransitive, the transitive and to a somewhat lesser extent the affective \isi{valency class}.

I will categorize verbs into \isi{valency classes} according to two main criteria: (i) the \isi{number} of arguments and (ii) the case marking of the subject-like argument. By `subject-like argument' I refer to the argument of the \isi{simple clause} that has the most subject properties as opposed to all other arguments (see \refsec{sec:Grammatical relations} for more details). Subject-like arguments are marked with one of the three cases \isi{absolutive}, \isi{ergative} or \isi{dative}. I use the terms ``one-place'' or ``monovalent'', ``two-place'' or ``bivalent'', and ``three-place'' or ``trivalent'' for referring to the \isi{number} of semantic arguments required by the verbs. The basic \isi{valency classes} and the case marking of the subject-like argument are summarized in \reftab{tab:Valency classes}.
...
\begin{table}
	...
	\begin{tabularx}{0.98\textwidth}[]{%
		...
		\midrule
			monovalent	&	intransitive (\refsec{sec:Intransitive verbs})		&	monovalent affective (\refsec{sec:Monovalent \isi{affective verbs} and exceptional monovalent constructions})	&	one verb \refex{ex:precipitation}\\	   
			...
			trivalent	&	\#	&		\#		&	ditransitive (\refsec{sec:Extended \isi{transitive verbs} and ditransitive verbs})\\ 
		\lspbottomrule
	\end{tabularx}
\end{table}

The term \sqt{extended intransitive} refers to two-place predicates that, in addition to an argument in the \isi{absolutive}, have a further argument in the \isi{dative} or another case; ``extended transitive'' verbs are \isi{three-place verbs} that besides having two arguments bearing the cases that are also used for \isi{transitive verbs}, have an additional argument marked with the \isi{dative} or in some other way \citet[122\tnd123]{Dixon1994}. Thus, extended \isi{transitive verbs} are ditransitive verbs. Furthermore, I use the term ``affective predicates'' for a clear-cut class of mostly \isi{experiential} predicates that express the \isi{experiencer} argument in the \isi{dative} and the \isi{stimulus} argument, if there is one, in the \isi{absolutive}. Affective verbs typically form their own \isi{valency class} in East Caucasian (see, e.g. \citealp{Comrie.vandenBerg2006}; \citealp{Ganenkov2006}; \citealp{Comrie.Forker.KhalilovaInPressb}). One might hypothesize that they belong to the class of extended \isi{intransitive verbs}. However, if one applies the commonly used test for subjecthood to the extended \isi{intransitive verbs} and the \isi{affective verbs} it immediately becomes clear that with the former class it is the \isi{absolutive} argument that exhibits most subject properties whereas with the latter class it is the \isi{dative} argument. For more information on grammatical relations in Sanzhi see \refsec{sec:Grammatical relations} and \citealp{Forker2019}.

\reftab{tab:Major valency classes} provides an overview of the major \isi{valency classes} discussed in this chapter; some minor classes are not listed, but discussed below. All verbs in the table and in the following subsections are presented in the order imperfective\slash perfective if they have two stems. Otherwise the single stem that is unspecified for aspect is given. In the table and in this chapter as well as elsewhere in the grammar I will use the following letters as mnemonics for macro roles (see \citet{Bickel2011} and \citet{Bickel.etal2015}): 
%
\begin{itemize}
...
\begin{table}
	...
	\begin{tabularx}{0.98\textwidth}[]{%
		...
		\lsptoprule
			case marking		&	\isi{number}~of		&	predicates\\
		...
		&	\tit{b-ubk'-\slash b-ebk'-} \sqt{die}; \tit{či-r-ha-b-ulq-\slash či-r-ha-b-uq-} \sqt{vomit} \refex{ex:My arms got tiredValency}, \refex{ex:If it was (the time) to die, I would have died long ago}\\\tablevspace
 			...
 			\multicolumn{3}{l}{{monovalent \isi{affective verbs} (\isi{dative})}}\\
			S-\tsc{dat}
		&	1
...
\begin{table}
	...
	\begin{tabularx}{\textwidth}[]{%
		...
		\midrule
			\isi{absolutive} P	&	y  \refex{ex:The people turned into stones}, \refex{ex:I will be 75 years old}		&	\mbox{y (transitive)}	&	y (affective)\\
			\isi{dative} P		&	y (extended intr.)		&	y					&	\#\\
			\isi{genitive} P	&	y	\refex{ex:The hips will be a strong medicine}			&	y					&	\#\\
...
Note that \reftab{tab:Case-marking of arguments of bivalent predicates} conflates basic \isi{valency classes} for bivalent predicates (i.e. extended intransitive, transitive, affective) with a \isi{number} of other special constructions, which are available for some predicates of the basic \isi{valency classes} (\isi{antipassive}, constructions with \isi{absolutive}, \isi{dative} or \isi{genitive} Ps).  
...
There are a variety of \isi{intransitive verbs} that are compounds and contain a nominal part (\refsec{ssec:compoundswithnouns}). The nominal part, however, does not function as argument of the verb. It most frequently appears in the \isi{absolutive} case \refex{ex:Our (people) were fasting like nowadays1}, but the \isi{genitive} case is also possible \refex{ex:He built a house and married.}, or the \tsc{loc}-lative or \isi{spatial postpositions}/adverbials. Note that in \refex{ex:Our (people) were fasting like nowadays1} the \isi{absolutive} argument that controls the agreement has been omitted and only its \isi{genitive} modifier appears in the clause.
...
The \isi{affective verbs} \sqt{see} and \sqt{hear} can also be used as \isi{monovalent verbs} with the meaning \sqt{be/become visible, show off} and \sqt{be/become audible} (see \refsec{sec:Argument control in complement constructions} for two example sentences and \refsec{sec:Bivalent affective verbs} below for a discussion).
...
\begin{exe}
	...
	\gll	du	at	r-iχči a-argu-d\\
		\tsc{1sg}	\tsc{2sg.dat}	\tsc{f-}believe \tsc{neg}-go.\tsc{ipfv-1}\\
...
There are a \isi{number} of \isi{compound verbs} and \isi{copula} constructions with \isi{experiential} semantics that belong to the extended intransitive class and mark the second argument with the \tsc{ante}-\isi{ablative} \refex{ex:\isi{experiential} compound verbs}, \refex{ex:Are you afraid of your wife? he says.} or take a clausal complement \refex{ex:We were embarrassed to talk a lot.} (see \refcpt{cpt:Complementation}).
%
\begin{exe}
	\ex	\label{ex:\isi{experiential} compound verbs}
	\begin{xlist}
...
\label{sec:Extended \isi{transitive verbs} and ditransitive verbs}

Extended \isi{transitive verbs} have three arguments bearing the \isi{ergative}, the \isi{absolutive} and a further case. They follow the same agreement rules as simple \isi{transitive verbs}. This means that the \isi{absolutive} argument triggers the \isi{gender}/\isi{number} agreement. Person agreement is controlled by the \isi{absolutive} or the \isi{ergative} argument, but never by the third argument (reciepient, addressee, etc.). It normally follows the person hierarchy 1, 2 > 3. The extended \isi{transitive verbs} in \refex{ex:extended di-transitive verbs} all have \isi{dative} arguments in addition to the \isi{ergative} and \isi{absolutive} arguments.
%
\begin{exe}
...
Another group of extended \isi{transitive verbs} are verbs of speech that take an addressee argument in the \tsc{in}-lative \xxref{ex:verbs of speech addressees}{ex:I asked Isakadi}. Their \isi{absolutive} argument is either a clause \refex{ex:Why did he not tell us that the water does not flow, they said}, \refex{ex:I asked Isakadi}, or a noun that refers to the speech event such as \textit{χabar} `story' \refex{ex:He tells the story}.
...
Verbs denoting movement and positioning of objects or animate entities combine with various \isi{spatial cases}, e.g. the \tsc{ad}-lative or the \tsc{loc}-lative.
%
\begin{exe}
...
Note that Sanzhi also has a range of \isi{compound verbs} with \isi{nouns} marked by the \isi{genitive} or by \isi{spatial postpositions}. However, the \isi{nouns} used in such constructions are not lexicalized objects but nominal parts of \isi{compound verbs}. For examples see \refsec{ssec:compoundswithnouns}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Bivalent affective verbs}
\label{sec:Bivalent affective verbs}
Bivalent \isi{affective verbs} are a relatively small class of \isi{two-place verbs} with epxeriential\slash affective semantics \refex{ex:bivalent affective verbs}. They express unintentional and uncontrollable perception, emotion, volition, cognitive activities and other non-agentive events and situations. 
...
Sanzhi Dargwa has a \isi{number} of \isi{labile verbs} that can be used as \isi{intransitive verbs} (with the corresponding morphology) or as \isi{transitive verbs}. Because in Sanzhi arguments can be omitted and are often omitted if their reference is clear from the context, at times it can be difficult to identify \isi{labile verbs}. Furthermore, occasionally \isi{transitive verbs} occur in impersonal constructions without arguments that could syntactically be defined as subjects or semantically identified as agents \refex{ex:It used to snow a lotA}, \refex{ex:He was not shaking like this (i.e. not able to move)}. Outside of the constructions shown in \refex{ex:It used to snow a lotA} and \refex{ex:He was not shaking like this (i.e. not able to move)}, the verb \tit{b-irq'-} (\tsc{ipfv})\slash\tit{b-arq'-} (\tsc{pfv}) is transitive and I therefore do not include it in the list of \isi{labile verbs}.
%
\begin{exe}
...
I found a few S=A \isi{labile verbs} that preserve the subject-like argument, namely:
%
\begin{exe}
...
Apart from being restricted to only one predicate class, namely canonical \isi{transitive verbs}, the \isi{antipassive} is additionally constrained in other ways:
%
\begin{enumerate}
...
Syntactically, the \isi{antipassive} is a detransitivizing operation. The main proof for this is, of course, that the A argument occurs in the \isi{absolutive} case and controls the \isi{gender} agreement, whereas the P argument takes the \isi{ergative} case. The functional range of the \isi{ergative} comprises not only the expression of agents, but also of other semantic roles with a more peripheral status (adjuncts), most notably instruments (\refsec{sssec:Ergative}). The \isi{ergative} P of the \isi{antipassive} largely fits into this range. Furthermore, the distinction between suffixes for intransitive and for \isi{transitive verbs} that is made in the \isi{imperative} and in the \isi{prohibitive} shows that verbs in the \isi{antipassive} construction are detransitivized. Thus, the \isi{prohibitive} suffixes for \isi{intransitive verbs} are \tit{-ut} (\tsc{sg})\slash\tit{-utːaja} (\tsc{pl}) with the stem augment vowel \tit{u}, whereas the \isi{transitive verbs} have \tit{-it} (\tsc{sg})\slash\tit{-itːaja} (\tsc{pl}) with the stem augment \textit{i} (\refsec{sec:prohibitive}). The \isi{antipassive} construction requires the same \isi{prohibitive} suffix as \isi{intransitive verbs} \refex{ex:\isi{antipassive} drink water}, which is ungrammatical in the \isi{ergative} construction \refex{ex: \isi{ergative} drink water}.
%
\begin{exe}
	\ex	\isi{antipassive} construction	\label{ex:\isi{antipassive} drink water}
	\begin{xlist}
	...
	\ex	\isi{ergative} construction	\label{ex: \isi{ergative} drink water}
	\begin{xlist}
...
In the Sanzhi corpus, causativized \isi{transitive verbs} are rather rare. Sentences \refex{ex:‎‎If it is like this, I will also make him record that other (story)} and \refex{ex:to make the boys drink} show two instances. Many corpus examples of \isi{causative constructions} have intransitive base verbs such as \refex{ex:This makes the mill spin around} and \refex{ex:‎Marijam was growing cucumbers} above, but causativized \isi{affective verbs} also occur frequently \refex{ex:(that) doctor Mahammad makes the people find (him)}.
%
\begin{exe}
...
The same option is available for the \isi{causative} of \sqt{know}, which translates as \sqt{tell, inform, make know} \refex{ex:‎He thanked the sun}. It is also possible for the \isi{experiencer} argument to change its case marking from \isi{dative} to \tsc{in}-lative because the latter case is regularly used for addressees with verbs of speech, but also for causees of causativized transitive and extended \isi{transitive verbs} \refex{ex:Allah, tell me what happened}, \refex{ex:(that) doctor Mahammad makes the people find (him)}.
%
\begin{exe}
...
Another way of forming \isi{causative constructions} is by means of transitive \isi{light verbs}. This operation is applied with \isi{compound verbs} that contain intransitive \isi{light verbs} (\refsec{sec:Formation of \isi{causative} verbs}).
