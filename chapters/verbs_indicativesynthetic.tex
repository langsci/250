...
The \isi{habitual present} is formed by adding person suffixes to the augmented stem of imperfective verbs (\reftab{tab:habitualpresent}). The third person has always the suffix \tit{-u}, which can thus be interpreted as a person marker, although it most probably originates from the stem augmentation vowel for \isi{intransitive verbs}. Alternatively there is the suffix \tit{-ar} for the third person (see below for a discussion). \reftab{tab:habitualpresent-examples} shows paradigms of three verbs. The \isi{intransitive verb} `say' is given in the female form for singular persons.
...
\begin{exe}
	...
	\gll	hel	prosto,	``dam	a-b-ikː-ar'', Ø-ik'-ul ca-w,	``ašːi-cːella ka-jž-ij w-elqː-un-ne=da''	Ø-ik'-ul	ca-w\\
		that	simply	\tsc{1sg.dat}	\tsc{neg-n-}want\tsc{.ipfv-prs.3}	\tsc{m-}say\tsc{.ipfv-icvb}	\tsc{cop-m}	\tsc{2pl-comit}	\tsc{down}-remain\tsc{.pfv-inf	} \tsc{m-}sate\tsc{.pfv-pret-cvb=1} \tsc{m-}say\tsc{.ipfv-icvb}	\tsc{cop-m}\\
