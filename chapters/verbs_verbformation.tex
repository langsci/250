...
%
\begin{itemize}
	\item	spatial \isi{preverbs} (treated in \refsec{sec:Preverbs})
...
In the majority of cases, causativization adds one argument to the valency frame of the base verb, i.e. \isi{intransitive verbs} become transitive and \isi{transitive verbs} become ditransitive.
...
In addition to morphological causativization, there are other formal means for making \isi{causative constructions} such as \isi{light verb} change and suppletion. This operation is applied to \isi{compound verbs}. Intransitive \isi{compound verbs} make use of the \isi{light verbs} \tit{b-irχʷ-} (\tsc{ipfv})\slash\tit{b-iχʷ-} (\tsc{pfv}) \sqt{be, become, can} \refex{ex:‎At that time they (the trousers) opened even more}. For causativization these \isi{light verbs} are replaced by \tit{b-irq'-} (\tsc{ipfv})\slash\tit{b-arq'-} (\tsc{pfv}) \sqt{do, make} \refex{ex:‎He opened the room together with them}. A full list of available \isi{light verbs} is given in \refsec{sec:Light verbs used in compounding and general remarks on compounds}.  
...
There are a fair \isi{number} of \isi{light verbs} occurring in \isi{compounding}. The most frequent ones are \tit{b-iχʷ-ij} \sqt{ be, become, can}, \tit{b-ik'ʷ-ij} \sqt{say} and \tit{b-arq'-ij} \sqt{do}. The verb \tit{b-ik'ʷ-ij} is widely used in compounds that denote verbs of speech and the production of other sounds, but also in many verbs of movement. Since it means \sqt{say} when used on its own, I stick to this as an overall gloss. The two tables below display intransitive (\reftab{tab:Intransitive light verbs}) and transitive (\reftab{tab:Transitive light verbs}) \isi{light verbs}.
%
\begin{table}
...
The verb-forming processes listed in the following sections of this Chapter are relatively freely combinable. If a specific combination is available depends largely on the semantics of the resulting verb. Thus, \isi{compound verbs} can contain spatial \isi{preverbs} \refexrange{ex:laughing about (me)}{ex:Do not look at them (the treesCOMPOUNDING} and causativized verb stems.
%
\begin{exe}
...
There are some \isi{nouns} that are particularly productive for the formation of compounds verbs and can combine with a variety of \isi{light verbs}. One is the noun \tit{ʁaj} \sqt{word, talk, language}, that occurs in the following compounds:
%
\begin{exe}
	\ex	intransitive \isi{compound verbs} (with subject in the \isi{absolutive})	\label{ex:wordcompoundsintransitive}
	\begin{xlist}
...
Finally, there are \isi{compound verbs} that contain \isi{nouns} with the encliticized postpositions \tit{=či} `on' and \tit{=(i)tːi} `after'.  These postpositions govern the \isi{genitive} or \isi{spatial cases} (\refsec{ssec:postposition hiti}, \refsec{ssec:postposition ci}), but when they are used in verbal \isi{compounding}, they are directly added to the \isi{nouns} without case marking:
...
The short adjectival stems (\refsec{sec:adjmorphclasses}) can easily occur in \isi{compound verbs} together with the \isi{light verbs} \tit{b-iχʷ-} (\tsc{pfv}) \sqt{be, become, can} \refex{ex:compound verbs with short adjectives intransitive become}, \tit{b-ik-} (\tsc{pfv}) \sqt{occur}  \refex{ex:compound verbs with short adjectives intransitive occur}, and \tit{b-arq'-} (\tsc{pfv}) \sqt{do, make} \refex{ex:compound verbs with short adjectives transitive}. These verbs occur in pairs of \isi{intransitive verbs} that normally have an inchoative meaning and \isi{transitive verbs} \refexrange{ex:when Nursijat gets better (healthy)}{ex:I will repair them}. 
%
\begin{exe}
...
Occasionally, other \isi{light verbs} are used, which leads to more idiosyncratic meanings \refex{ex:This is a beautiful, pleasant place}:
%
\begin{exe}
...
\begin{table}
	...
	\begin{tabularx}{1.0\textwidth}[]{%
		...
		\lsptoprule
			\isi{ideophone}					&	\isit{light verb}						&	translation of \isit{compound verb}\\
		\midrule
...
There is a closed class of bound lexemes that occur only in \isi{compound verbs} and of which thus the meaning out of the context of a \isi{compound verb} is impossible to determine. These items do not belong to any of the lexical categories that Sanzhi has. Some of the bound stems are flexible with respect to the \isi{light verbs} with which they combine leading to a variety of different \isi{compound verbs} containing the same bound stem (\reftab{tab:Compound verbs with bound lexical stems (Part 1)}).
%
\begin{table}
...
As with the \isi{compound verbs} containing short \isi{adjectives} (\refsec{sec:compoundswithshortadjectives}), there are often pairs of intransitive and \isi{transitive verbs}. They can be divided into groups depending on the \isi{intransitive verbs} that they make use of.
%
Firstly, there are bound stems that are combined with \tit{b-iχʷ-} (\tsc{pfv}) \sqt{be, become, can} and \tit{b-arq'-} (\tsc{pfv}) \sqt{do, make} to form intransitive and \isi{transitive verbs}, see, for instance, the examples in \refex{ex:hajbars}. Other \isi{light verbs} cannot be used together with these stems.
%
\begin{exe}
	\ex	\label{ex:hajbars}
	\begin{xlist}
...
And secondly, there are bound stems that are combined with \tit{b-ik-} (\tsc{pfv}) \sqt{occur} and \tit{b-arq'-} (\tsc{pfv}) \sqt{do, make} to form intransitive and \isi{transitive verbs}:
%
\begin{exe}
	\ex
	\begin{xlist}
